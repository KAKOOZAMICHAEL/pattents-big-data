...
def main():
    ...
    patent_file = BASE_DIR / "g_abstract" / "g_patent.tsv"
    ...
    abstract_file = BASE_DIR / "g_patent" / "g_patent_abstract.tsv"
    ...
    for chunk in pd.read_csv(patent_file, sep="\t", chunksize=chunk_size, usecols=patent_cols, dtype=str, on_bad_lines='skip', engine='python', quoting=3, quotechar='"'):
        ...
        # Store valid IDs
        valid_patents.update(filtered['"patent_id"'].str.strip('"').dropna().tolist())
